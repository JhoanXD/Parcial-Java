...
String imagenPrincipalAlt = null;
try {
...
<main class="container py-5">
  ...
  <div class="row g-5 mt-2 align-items-start">
    <div class="col-lg-7"><div class="bg-light border rounded-3 p-3" style="min-height:420px;"><% try { Class.forName("com.mysql.cj.jdbc.Driver"); try (Connection imagenesConexion = abrirConexion(); PreparedStatement imagenesSentencia = imagenesConexion.prepareStatement("SELECT url, texto_alternativo FROM imagen_propiedad WHERE id_propiedad=? ORDER BY orden_imagen")) { imagenesSentencia.setInt(1, idPropiedad); try (ResultSet imagenes = imagenesSentencia.executeQuery()) { while (imagenes.next()) { imagenesCount++; if (imagenPrincipal == null) { imagenPrincipal = imagenes.getString("url"); imagenPrincipalAlt = imagenes.getString("texto_alternativo"); } } } } } catch (Exception ignored) { } %><% if (imagenPrincipal != null) { %><div class="mb-3"><img src="<%= imagenPrincipal %>" alt="<%= imagenPrincipalAlt %>" class="img-fluid rounded w-100" style="height:420px;object-fit:cover"></div><% } else { %><div class="text-center py-5"><i class="bi bi-building text-primary" style="font-size:8rem;"></i></div><% } %><% if (imagenesCount > 1) { %><div class="row g-3"><% try { Class.forName("com.mysql.cj.jdbc.Driver"); try (Connection imagenesConexion = abrirConexion(); PreparedStatement imagenesSentencia = imagenesConexion.prepareStatement("SELECT url, texto_alternativo FROM imagen_propiedad WHERE id_propiedad=? ORDER BY orden_imagen")) { imagenesSentencia.setInt(1, idPropiedad); try (ResultSet imagenes = imagenesSentencia.executeQuery()) { int indice = 0; while (imagenes.next()) { indice++; if (indice == 1) continue; %><div class="col-md-6"><img src="<%= imagenes.getString("url") %>" alt="<%= imagenes.getString("texto_alternativo") %>" class="img-fluid rounded w-100" style="height:190px;object-fit:cover"></div><% } } } } catch (Exception ignored) { } %></div><% } %></div><% if ("INMOBILIARIA".equals(session.getAttribute("usuarioRol"))) { %><a href="../imagenes/imagenes_propiedad.jsp?id=<%= idPropiedad %>&matricula=<%= matricula %>" class="btn btn-sm btn-outline-primary mt-2">Gestionar imágenes</a><% } %></div>
    <div class="col-lg-5"><span class="badge <%= "VENTA".equals(modalidad) ? "text-bg-secondary" : "text-bg-dark" %> mb-3"><%= modalidad %></span><p class="text-muted small mb-2">Matrícula <%= matricula %> | <%= ciudad %> | <%= tipo %></p><h1 class="display-6 fw-bold"><%= titulo %></h1><p class="fs-3 fw-bold text-primary my-4"><%= precio %></p><div class="row g-3 border-top border-bottom py-3 mb-4"><div class="col-4"><strong class="d-block"><%= habitaciones %></strong><small class="text-muted">Habitaciones</small></div><div class="col-4"><strong class="d-block"><%= banos %></strong><small class="text-muted">Baños</small></div><div class="col-4"><strong class="d-block"><%= area %> m2</strong><small class="text-muted">Área</small></div></div><p class="text-muted"><%= descripcion %></p><p class="small text-muted mb-3">Publica: <%= inmobiliaria %></p><% if ("ok".equals(mensajeFavorito)) { %><div class="alert alert-success small">Propiedad guardada en favoritos.</div><% } else if ("error".equals(mensajeFavorito)) { %><div class="alert alert-danger small">No fue posible guardar el favorito.</div><% } %><form method="post" action="../cliente/favorito.jsp" class="mb-2"><input type="hidden" name="id_propiedad" value="<%= idPropiedad %>"><input type="hidden" name="matricula" value="<%= matricula %>"><button class="btn btn-outline-primary btn-lg w-100" type="submit"><i class="bi bi-heart me-2"></i>Guardar en favoritos</button></form><div class="row g-2"><div class="col-6"><a href="../cliente/cita.jsp?id_propiedad=<%= idPropiedad %>&matricula=<%= matricula %>" class="btn btn-primary w-100"><i class="bi bi-calendar-check me-1"></i>Agendar</a></div><div class="col-6"><a href="../cliente/solicitud.jsp?id_propiedad=<%= idPropiedad %>&matricula=<%= matricula %>" class="btn btn-outline-dark w-100"><i class="bi bi-file-earmark-text me-1"></i>Solicitar</a></div></div></div>
  </div>
