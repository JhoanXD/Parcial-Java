...
try {
    ...
    try (Connection conexion = abrirConexion()) {
        ...
        int idPropiedad = 0;
        try (PreparedStatement sentencia = conexion.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS)) {
            sentencia.setInt(1, idInmobiliaria);
            sentencia.setInt(2, Integer.parseInt(request.getParameter("id_ciudad")));
            sentencia.setInt(3, Integer.parseInt(request.getParameter("id_tipo")));
            sentencia.setString(4, request.getParameter("matricula"));
            sentencia.setString(5, request.getParameter("titulo"));
            sentencia.setString(6, request.getParameter("descripcion"));
            sentencia.setString(7, request.getParameter("modalidad"));
            sentencia.setBigDecimal(8, new java.math.BigDecimal(request.getParameter("precio")));
            sentencia.setBigDecimal(9, new java.math.BigDecimal(request.getParameter("area_m2")));
            sentencia.setInt(10, Integer.parseInt(request.getParameter("habitaciones")));
            sentencia.setInt(11, Integer.parseInt(request.getParameter("banos")));
        ...
        if (idPropiedad > 0) {
            ...
            if (archivo != null && archivo.getSize() > 0) {
                ...
                try (PreparedStatement imagen = conexion.prepareStatement("INSERT INTO imagen_propiedad (id_propiedad,url,texto_alternativo,orden_imagen) VALUES (?, ?, ?, ?)")) {
                    ...
                    imagen.setString(3, nombreOriginal);
                    imagen.setInt(4, 1);
                    imagen.executeUpdate();
                }
            }
        }
    }
    response.sendRedirect("inmobiliaria_propiedades.jsp?estado=ok");
} catch (SQLException exception) { response.sendRedirect("propiedad_nueva.jsp?error=" + (exception.getErrorCode() == 1062 ? "duplicado" : (exception.getErrorCode() == 1452 ? "referencia" : "error"))); } catch (NumberFormatException exception) { response.sendRedirect("propiedad_nueva.jsp?error=campos"); } catch (IllegalArgumentException exception) { response.sendRedirect("propiedad_nueva.jsp?error=archivo"); } catch (Exception exception) { response.sendRedirect("propiedad_nueva.jsp?error=error"); }
%>
